...
import { Moon, Sun, Play, Pause, SkipBack, SkipForward, QrCode, Volume2, VolumeX } from "lucide-react"
...
export default function Home() {
  ...
  const handleQrSuccess = (result: string) => {
    ...
    if (result.includes("verse=")) {
      ...
      if (!isNaN(verseNumber) && verseNumber >= 0 && verseNumber < verses.length) {
  ...
  return (
    <main className={`min-h-screen flex flex-col relative ${theme === "dark" ? "text-white" : "text-black"}`}>
      ...
      <div className="fixed inset-0 z-0">
        <Image src="/mosque-background.jpg" alt="Mosque Background" fill className="object-cover" priority />
      ...
      <header className="relative z-10 p-4 flex justify-between items-center">
        <div className="flex items-center">
          <Image src="/logo.png" alt="Ayati Link" width={150} height={60} className="object-contain" />
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative z-10 px-4 py-8">
        <div className="max-w-3xl w-full bg-background/80 backdrop-blur-sm rounded-lg p-6 shadow-lg">
          <h2 className="text-center text-xl font-medium mb-2">{currentVerse.surah}</h2>
          <h3 className="text-center text-lg mb-6">Verset {currentVerse.verseNumber}</h3>

          <p className="text-right font-arabic text-2xl leading-loose mb-6 rtl">{currentVerse.arabic}</p>

          <p className="text-lg mb-8">{currentVerse.french}</p>
      ...
      <footer className="relative z-10 p-4 text-center text-sm">
        ...
        <p>A project by AYATI LINK © {new Date().getFullYear()}</p>
      </footer>
      ...
      {showScanner && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
          <div className="bg-background rounded-lg max-w-md w-full p-4">
            ...
            <QrScanner onResult={handleQrSuccess} onClose={() => setShowScanner(false)} />
          </div>
        </div>
      )}
      ...
      {showQrCode && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
          <div className="bg-background rounded-lg max-w-md w-full p-4">
            ...
            <QrGenerator
              url={typeof window !== "undefined" ? window.location.href : "https://ayati-link.vercel.app"}
